...
function openModule() {
    "use strict";
    var protectedScope, initMethods = [], webaudioUnlocked = false, src, context, gainNode, compressor, sampleIndex = 0, compressorParams = ["threshold", "knee", "ratio", "reduction", "attack", "release"], ua = navigator.userAgent, os, browser, legacy = false;
    ...
    */
    exports.sequencer = sequencer = {
        ...
        unlockWebAudio: function () {
            ...
            var src = context.createOscillator(), gainNode = context.createGainNode();
...
function assetManager() {
    ...
    busy = false, taskIndex = 0, finishedTasks = {}, taskQueue = [], callbacks = [];
    sequencer.removeMidiFile = function (path) {
        var item, items = [], i, folder;
    ...
    sequencer.removeSamplePack = function (path) {
        var item, items = [], i, samples, sample, s, folder;
...
function assetPack() {
    ...
    var index = 0, storage, // defined in open_module.js
    ...
    function store(assetpack) {
        var occupied = findItem(assetpack.localPath, sequencer.storage.assetpacks, true), action = assetpack.action;
    ...
    function loadLoop(assetpack, callback) {
        var i, assets, asset, loaded = store(assetpack), localPath = assetpack.localPath;
        if (loaded === true) {
        ...
        if (assetpack.url !== undefined) {
            var packs = sequencer.storage.assetpacks, tmp, p, double = null;
            for (p in packs) {
                tmp = packs[p];
    ...
    sequencer.addAssetPack = function (config, callback) {
        var type = typeString(config), assetpack, json, name, folder;
        if (type !== "object") {
            if (sequencer.debug >= 2) {
...
function audioEvent() {
    ...
    AudioEvent.prototype.clone = AudioEvent.prototype.copy = function () {
        var event = new AudioEvent(), property;
...
function audioRecorder() {
    ...
    microphoneAccessGranted = null, localMediaStream, bufferSize = 8192, millisPerSample, bufferMillis, waveformConfig = {
        height: 200,
        width: 800,
        //density: 0.0001,
        sampleStep: 1,
    ...
    function createAudioBuffer(scope, wavArrayBuffer, interleavedSamples, planarSamples, type) {
        var i, frameCount = planarSamples.length, base64 = encode64(wavArrayBuffer), audioBuffer = context.createBuffer(1, frameCount, context.sampleRate), samples = audioBuffer.getChannelData(0), recording = {
            id: scope.recordId,
            audioBuffer: null,
        ...
        for (var i_1 = 0; i_1 < frameCount; i_1++) {
            samples[i_1] = planarSamples[i_1];
        }
    ...
    function encodeAudioBuffer(scope, wavArrayBuffer, interleavedSamples, type) {
        ...
        context.decodeAudioData(wavArrayBuffer, function (audioBuffer) {
            var base64 = encode64(wavArrayBuffer), recording = {
                id: scope.recordId,
            ...
            getWaveformData(audioBuffer, scope.waveformConfig, 
            //callback
            function (data) {
    ...
    AudioRecorder.prototype.start = function () {
        var scope = this, song = this.track.song;
    ...
    AudioRecorder.prototype.createAudio = function () {
        ...
        var bufferIndexStart = parseInt((this.song.metronome.precountDurationInMillis + this.song.audioRecordingLatency) /
            millisPerSample), bufferIndexEnd = -1;
    ...
    AudioRecorder.prototype.setAudioRecordingLatency = function (recordId, value, callback) {
        var bufferIndexStart = parseInt(value / millisPerSample), bufferIndexEnd = -1;
        this.callback = callback;
        this.worker.postMessage({
...
function audioTrack() {
    ...
    AudioTrack.prototype.allNotesOff = function () {
        var sampleId, sample, scheduledSamples = this.scheduledSamples;
...
function channelEffects() {
    "use strict";
    var id = 0, context, zeroValue = 0.00000000000000001, createClass, // defined in util.js
    getSample, // defined in instrument_manager.js
    Reverb, Panner, Panner2, Delay, BiQuadFilter, Compressor;
    ...
    sequencer.protectedScope.addInitMethod(function () {
        ...
        Panner2.prototype.setPosition = function (value) {
            var xDeg = parseInt(value), zDeg = xDeg + 90, x, y, z;
            if (zDeg > 90) {
                zDeg = 180 - zDeg;
            }
...
function eventStatistics() {
    ...
    supportedOperators = "max min avg all", supportedProperties = "data1 data2 velocity noteNumber noteName frequency", 
    //public
    getStats;
    /**
          @memberof sequencer
      ...
      */
    getStats = function () {
        var args = Array.prototype.slice.call(arguments), numArgs = args.length, property, operator, events, searchPattern, patternLength, i, maxi, event, propValue, minNoteName, maxNoteName, min = 128, //Number.MAX_VALUE,
        max = -1, sum = 0, avg = 0, useNoteName = false;
...
function findEvent() {
    ...
    //local
    patterns, operators, properties = {
        barsbeats: ["bar", "beat", "sixteenth", "tick"],
        time: ["hour", "minute", "second", "millisecond"],
    }, logicalOperators = "OR AND NOT XOR", logicalOperatorsRegex = new RegExp(" " + logicalOperators.replace(/\s+/g, " | ") + " "), // → replaces logical operator by a white space
    supportedProperties = {
        bar: 1,
        beat: 1,
        sixteenth: 1,
    ...
    findEvent, findNote, 
    //private
    getEvents, checkValue, createPattern, checkOperators, checkCondition, checkCondition2, inverseOperator, removeMutualEvents, removeDoubleEvents, performSearch;
    /*
    ...
    findEvent = function () {
        ...
        var args = Array.prototype.slice.call(arguments), i, maxi, 
        //j, maxj,
        //k, maxk,
        searchString, tmp, operator, pattern, prevPattern, prevOperator, patternIndex, operatorIndex, events, results, lastResult, subResult1, subResult2;
        //console.log(args[0])
        events = getEvents(args[0]);
        results = [];
        if (events.length === 0) {
    ...
    removeMutualEvents = function (resultSet1, resultSet2) {
        var i, maxi = resultSet1.length, j, maxj = resultSet2.length, event, eventId, addEvent, result = [];
        for (i = 0; i < maxi; i++) {
            addEvent = true;
            event = resultSet1[i];
            eventId = event.id;
    ...
    removeDoubleEvents = function (events) {
        var i, maxi = events.length, event, eventId, lastId, result = [];
    ...
    performSearch = function (events, pattern, inverse) {
        var searchResult = [], property = pattern.property, operator1 = pattern.operator1, operator2 = pattern.operator2, value1 = pattern.value1, value2 = pattern.value2, numEvents = events.length, event, i, condition = false;
    ...
    checkCondition = function (property, propValue, operator1, value1, operator2, value2) {
        var result = false, isString = false;
        if (propValue === undefined) {
            return result;
        }
        switch (property) {
    ...
    createPattern = function (args) {
        var pattern = {
            ...
            operator2: args[3],
            value2: args[4],
        }, property = args[0], operator1 = args[1], value1 = args[2], operator2 = args[3], value2 = args[4], i;
    ...
    checkOperators = function (pattern) {
        var operator1 = pattern.operator1, operator2 = pattern.operator2, check = function (operator) {
    ...
    findNote = function () {
        var results = findEvent.apply(this, arguments), numEvents = results.length, i, event, noteOnEvent, noteOnEvents = {}, tmp, resultsFiltered = [];
...
function instrument() {
    ...
    Instrument.prototype.reset = function () {
        var instrument = sequencer.getInstrument(this.config.localPath), samplepack = sequencer.getSamplePack(this.config.sample_path);
    ...
    Instrument.prototype.parse = function () {
        var i, maxi, v, v1, v2, length, octave, note, noteName, noteNumber, pathArray, buffer, names, id, data, subdata, update, sample, sampleConfig, samplePack, audioFolder, config = this.config, noteNameMode = config.notename_mode === undefined ? sequencer.noteNameMode : config.notename_mode, mapping = config.mapping, me = this;
    ...
    Instrument.prototype.getInfoAsHTML = function () {
        var html = "", instrumentInfo = "", samplepackInfo = "", sp = this.samplepack;
        ...
        instrumentInfo +=
            "<tr><td>keyrange</td><td>" +
                ...
                ")";
        instrumentInfo +=
            " - " + this.highestNote + "(" + sequencer.getFullNoteName(this.highestNote) + ")</td></tr>";
        if (instrumentInfo !== "") {
        ...
        if (sp.filesize !== undefined) {
            samplepackInfo +=
                "<tr><td>filesize</td><td>" + round(sp.filesize / 1024 / 1024, 2) + " MiB</td></tr>";
        }
        if (samplepackInfo !== "") {
    ...
    Instrument.prototype.createSample = function (event) {
        var noteNumber = event.noteNumber, velocity = event.velocity, data = this.sampleDataByNoteNumber[noteNumber], type = typeString(data);
        if (type === "array") {
            data = data[velocity];
    ...
    Instrument.prototype.setKeyScalingPanning = function (start, end) {
        ...
        var i, data, numSamples = this.sampleData.length, panStep, currentPan;
        if (start === false) {
    ...
    Instrument.prototype.setKeyScalingRelease = function (start, end, envelope) {
        var i, data, numSamples = this.sampleData.length, releaseStep, currentRelease;
        this.releaseEnvelope = envelope || this.releaseEnvelope;
        if (isNaN(start) === false && isNaN(end) === false) {
    ...
    Instrument.prototype.processEvent = function (midiEvent) {
        ...
        var type = midiEvent.type, data1, data2, track, output;
    ...
    Instrument.prototype.stopSustain = function (seconds) {
        var midiNote, scheduledSamples = this.scheduledSamples, sustainPedalSamples = this.sustainPedalSamples;
    ...
    Instrument.prototype.stopNote = function (midiEvent) {
        ...
        var sourceId = midiEvent.midiNote.id, sample = this.scheduledSamples[sourceId], scheduledSamples = this.scheduledSamples, sustainPedalSamples = this.sustainPedalSamples;
    ...
    Instrument.prototype.reschedule = function (song) {
        var min = song.millis, max = min + sequencer.bufferTime * 1000, max2 = min + 20, scheduledSamples = this.scheduledSamples, id, note, sample;
        for (id in scheduledSamples) {
            if (scheduledSamples.hasOwnProperty(id)) {
                ...
                else if (note.noteOn.millis >= min &&
    ...
    Instrument.prototype.unschedule = function () {
        var args = Array.prototype.slice.call(arguments), events = [], i, e, id, sample;
    ...
    Instrument.prototype.allNotesOff = function () {
        var sample, sampleId, scheduledSamples = this.scheduledSamples;
        this.stopSustain(0);
        this.sustainPedalDown = false;
    ...
    sequencer.createInstrument = function (arg) {
        var type = typeString(arg), config, instrument;
...
function instrumentConfig() {
    "use strict";
    var ajax, // defined in utils.js
    parseUrl, // defined in utils.js
    findItem, // defined in utils.js
    storeItem, // defined in utils.js
    typeString, // defined in utils.js
    objectForEach, // defined in utils.js
    index = 0, InstrumentConfig;
    ...
    function store(instrument) {
        var occupied = findItem(instrument.localPath, sequencer.storage.instruments, true), action = instrument.action;
    ...
    sequencer.addInstrument = function (config, callback, callbackAfterAllTasksAreDone) {
        var type = typeString(config), instrument, json, name, folder;
        if (type !== "object") {
            if (sequencer.debug >= 2) {
...
function keyEditor() {
    "use strict";
    var //private
    KeyEditor, updateDataKeys = "newEvents newNotes newParts changedEvents changedNotes changedParts removedEvents removedNotes removedParts".split(" "), 
    // default values
    tickWidth = 0.1, pitchHeight = 10, barsPerPage = 4, snapValueX = 0, // means snaps is off -> ticks value do not get rounded
    //snapValueX = 1, // means snaps to all ticks
    snapValueY = "chromatic", eventWidth = 2, ceil = Math.ceil, 
    //import
    createIteratorFactory, getPosition, createPlayhead, getScaffoldingBars, typeString, objectToArray, arrayToObject, debug, round, floor, createNote, 
    //public
    //getLines,
    //xToTicks,
    //yToPitch,
    //private
    setPageData, checkNextPage, checkScrollPosition, dispatchEvent, handleKeys;
    ...
    KeyEditor.prototype.addEventListener = function (id, cb) {
        var ids = id.split(" "), tmp, editor = this, eventId;
    ...
    KeyEditor.prototype.scroll = function (action) {
        ...
        var x, tmp = round(this.scrollX / (this.viewportWidth / this.barsPerPage));
        this.scrollPosition = ((this.viewportWidth / this.barsPerPage) * tmp) / this.viewportWidth;
        switch (action) {
            case ">":
    ...
    KeyEditor.prototype.getEventRect = function (event) {
        ...
        var x = this.ticksToX(event.ticks - this.startTicks, false), y = this.pitchToY(event.number), w = eventWidth * this.tickWidth, h = this.pitchHeight;
        return {
            x: x,
            y: y,
            width: w,
    ...
    KeyEditor.prototype.getNoteRect = function (note) {
        ...
        y = this.pitchToY(note.number), w = note.durationTicks * this.tickWidth, h = this.pitchHeight, start, end, diff;
        if (note.endless) {
    ...
    KeyEditor.prototype.moveNote = function (x, y) {
        ...
        var newPitch = this.yToPitch(y).number, oldPitch = this.selectedNote.pitch, newTicks = this.xToTicks(x - this.gripX), oldTicks = this.selectedNote.ticks, part = this.selectedNote.part, update = false;
    ...
    KeyEditor.prototype.movePart = function (x, y, autoUpdate) {
        ...
        var newPitch = this.yToPitch(y).number, oldPitch = this.selectedPart.pitch, newTicks = this.xToTicks(x - this.gripX), oldTicks = this.selectedPart.ticks, update = false;
        if (newPitch !== oldPitch) {
    ...
    KeyEditor.prototype.getEventAt = function (x, y) {
        var position = this.getSongPosition(x), pitch = this.getPitchAt(y);
    };
    KeyEditor.prototype.getEventsInRect = function (x, y, w, h) {
        var startPos = this.getSongPosition(x), endPos = this.getSongPosition(x + w), startPitch = this.getPitchAt(y + h), endPitch = this.getPitchAt(y);
    };
    ...
    KeyEditor.prototype.getSnapshot = function () {
        var activeEventsObj, activeNotesObj, activePartsObj, recordedNotesSong, 
        //recordingNotesSong,
        recordedEventsSong, nonActiveEvents = [], nonActiveNotes = [], nonActiveParts = [], prevActiveEvents = [].concat(this.activeEvents), prevActiveNotes = [].concat(this.activeNotes), prevActiveParts = [].concat(this.activeParts), recordedEvents = [], recordedNotes = [], recordingNotes = [], 
        //prevRemovedNotes = [].concat(this.removedNotes),
        s, e, n, p, i, j, tmp, length, startBar, endBar;
        this.activeEvents = [];
        this.activeNotes = [];
        this.activeParts = [];
    ...
    handleKeys = function (editor) {
        var p = editor.selectedPart, n = editor.selectedNote;
        if (p !== undefined) {
            p.track.removePart(p);
            this.song.update();
        }
...
function keyEditorIteratorFactory() {
    "use strict";
    var minWidthSixteenth = 0.042, minWidthBeat = 0.02, 
    /*
            events,
            numEvents,
            notes,
    ...
    */
    ...
    Factory.prototype.createVerticalLineIterator = function () {
        var supportedTypes = "bar beat sixteenth", lineType, numTicks = {}, tickWidth, offset, type, ticks, endTicks, bar, beat, sixteenth, nominator, numSixteenth, startPosition, endPosition, editor = this.editor, position = this.position, 
        // widthBar,
        // widthBeat,
        // widthSixteenth,
        ...
        hasNext = function (t) {
            var diffTicks = endTicks - ticks, result = false;
            if (t) {
                type = t;
                if (tickWidth < minWidthBeat) {
                    type = "bar";
    ...
    Factory.prototype.createHorizontalLineIterator = function () {
        var index, pitch, range, pitchHeight, data = {}, editor = this.editor, next, hasNext, reset;
        next = function (type) {
            data = {
    ...
    Factory.prototype.createEventIterator = function () {
        var startTicks, endTicks, hasNextCalled, index, nextEvent, editor = this.editor, position = this.position, events = this.events, numEvents = this.numEvents, types = "", next, hasNext, reset, setTypes;
        hasNext = function (t) {
            types = t || types;
            hasNextCalled = true;
            index++;
    ...
    Factory.prototype.createNoteIterator = function () {
        var startTicks, endTicks, hasNextCalled, index, newNote, nextNote, editor = this.editor, song = this.song, notes = this.notes, numNotes = this.numNotes, types = "", next, hasNext, reset, setTypes;
        hasNext = function (t) {
            types = t || types;
            hasNextCalled = true;
            index++;
    ...
    Factory.prototype.createPartIterator = function () {
        var index, max, part, data = {}, editor = this.editor, song = this.song, parts = this.parts, next, hasNext, reset;
        next = function (type) {
            part = parts[index++];
...
function metronome() {
    ...
    function createEvents(metronome, startBar, endBar, id) {
        var i, j, data, velocity, noteLength, noteNumber, beatsPerBar, ticksPerBeat, ticks = 0, events = [], song = metronome.song, noteOn, noteOff, note;
        //console.log(startBar, endBar);
    ...
    Metronome.prototype.getPrecountEvents = function (maxtime) {
        var events = this.precountEvents, maxi = events.length, i, event, result = [];
...
function midiEvent() {
    ...
    MidiEvent.prototype.clone = MidiEvent.prototype.copy = function () {
        var event = new MidiEvent(), property;
        for (property in this) {
            if (this.hasOwnProperty(property)) {
                //console.log(property);
    ...
    sequencer.createMidiEvent = function () {
        /**
                ...
                @param data2 {int}
            */
        var args = slice.call(arguments), className = args[0].className;
...
function midiFile() {
    ...
    function parse(midifile, buffer, callback) {
        ...
        var data, i, j, numEvents, part, track, numTracks, events, event, ticks, tmpTicks, channel, parsed, timeEvents, noteNumber, bpm, lastNoteOn, lastNoteOff, ppqFactor, type, lastType, lastData1, lastData2, numNoteOn, numNoteOff, numOther, noteOns, noteOffs;
        ...
        ajax({
            ...
            onSuccess: function (data) {
                if (midifile.responseType === "json") {
                    ...
                    midifile.localPath =
                        midifile.folder !== undefined ? midifile.folder + "/" + midifile.name : midifile.name;
    ...
    function store(midifile) {
        var occupied = findItem(midifile.localPath, sequencer.storage.midi, true), action = midifile.action;
    ...
    MidiFile = function (config) {
        ...
        if (this.url !== undefined) {
            this.responseType =
                this.url.indexOf(".json") === this.url.lastIndexOf(".") ? "json" : "arraybuffer";
        }
        else {
    ...
    sequencer.addMidiFile = function (config, callback) {
        var type = typeString(config), midifile, json, name, folder;
        if (type !== "object") {
            if (sequencer.debug >= 2) {
...
function midiNote() {
    ...
    MidiNote = function (args) {
        var numArgs = args.length, on, off, startTicks, endTicks, noteNumber, velocity;
        //console.log(args);
        if (numArgs === 1) {
            on = args[0];
            if (on === undefined) {
...
function midiParse() {
    "use strict";
    var lastEventTypeByte, trackName, instrumentName, 
    //import
    ...
    function parseStream(stream) {
        var formatType, trackCount, timeDivision, ticksPerBeat, tracks = [], i, trackNames = [], trackChunk, trackStream, headerChunk, headerStream;
...
function midiSystem() {
    "use strict";
    var context, // defined in open_module.js
    typeString, // defined in util.js
    objectForEach, // defined in util.js
    createMidiNote, // defined in midi_note.js
    createMidiEvent, // defined in midi_event.js
    slice = Array.prototype.slice, songMidiEventListener, midiAccess, midiInputsOrder, midiOutputsOrder, midiInitialized = false, midiEventListenerId = 0;
    ...
    function setMidiInputSong(id, flag, song) {
        var input = sequencer.midiInputs[id], tracks = song.tracks, maxi = song.numTracks - 1, i, track;
    ...
    function setMidiOutputSong(id, flag, song) {
        var output = sequencer.midiOutputs[id], tracks = song.tracks, maxi = song.numTracks - 1, i, track, time;
        flag = flag === undefined ? true : flag;
        if (output === undefined) {
            if (sequencer.debug === true) {
    ...
    function handleMidiMessageSong(midiMessageEvent, song, input) {
        var data = midiMessageEvent.data, i, track, tracks = song.tracks, numTracks = song.numTracks, midiEvent, listeners;
    ...
    function handleMidiMessageTrack(midiEvent, track, input) {
        var song = track.song, note, listeners, channel;
    ...
    function addMidiEventListener(args, obj) {
        ...
        var id = midiEventListenerId++, types = {}, ids = [], listener, loop;
        // should I inline this?
        loop = function (args, i, maxi) {
            for (i = 0; i < maxi; i++) {
                var arg = args[i], type = typeString(arg);
                //console.log(type);
                if (type === "array") {
                    loop(arg, 0, arg.length);
                }
    ...
    function getMidiPortsAsDropdown(config, obj) {
        var select = document.createElement("select"), option, ports, type = config.type, id = config.id || type, div = config.div, firstOption = config.firstOption;
...
} //http://www.deluge.co/?q=midi-tempo-bpm // This code is based on https://github.com/sergi/jsmidi
/*
...
*/ function midiWrite() {
    "use strict";
    var AP = Array.prototype, PPQ = sequencer.defaultPPQ, HDR_CHUNKID = ["M".charCodeAt(0), "T".charCodeAt(0), "h".charCodeAt(0), "d".charCodeAt(0)], HDR_CHUNK_SIZE = [0x0, 0x0, 0x0, 0x6], // Header size for SMF
    ...
    HDR_PPQ = str2Bytes(PPQ.toString(16), 2), TRK_CHUNKID = ["M".charCodeAt(0), "T".charCodeAt(0), "r".charCodeAt(0), "k".charCodeAt(0)], 
    // Meta event codes
    META_SEQUENCE = 0x00, META_TEXT = 0x01, META_COPYRIGHT = 0x02, META_TRACK_NAME = 0x03, META_INSTRUMENT = 0x04, META_LYRIC = 0x05, META_MARKER = 0x06, META_CUE_POINT = 0x07, META_CHANNEL_PREFIX = 0x20, META_END_OF_TRACK = 0x2f, META_TEMPO = 0x51, META_SMPTE = 0x54, META_TIME_SIG = 0x58, META_KEY_SIG = 0x59, META_SEQ_EVENT = 0x7f;
    function write(song) {
        var byteArray = [].concat(HDR_CHUNKID, HDR_CHUNK_SIZE, HDR_TYPE1), tracks = song.tracks, numTracks = song.tracks.length + 1, i, maxi, track, midiFile, destination, b64, arrayBuffer, dataView, uintArray;
    ...
    function trackToBytes(events, lastEventTicks, trackName, instrumentName) {
        var lengthBytes, i, maxi, event, status, trackLength, // number of bytes in track chunk
        ticks = 0, delta = 0, trackBytes = [];
        if (trackName) {
...
function musicXMLParser() {
    ...
    function parse(xml) {
        var parser = new DOMParser(), xmlDoc = parser.parseFromString(xml, "application/xml"), type = xmlDoc.firstChild.nextSibling.nodeName;
    ...
    function parsePartWise(xmlDoc) {
        var partIterator = xmlDoc.evaluate("//score-part", xmlDoc, nsResolver, XPathResult.ANY_TYPE, null), partNode, measureIterator, measureNode, noteIterator, noteNode, measureNumber, tracks = [], timeEvents = [], tiedNotes = {}, tieStart, tieStop, tieIterator, tieNode, events, song, track, part, noteOn, noteOff, name, id, tmp1, tmp2, step, alter, octave, voice, noteType, noteDuration, noteName, noteNumber, velocity, rest, chord, divisions, numerator, denominator, ppq = sequencer.defaultPPQ, ticks;
        while ((partNode = partIterator.iterateNext()) !== null) {
            ...
            name = xmlDoc.evaluate("part-name", partNode, nsResolver, XPathResult.STRING_TYPE, null)
                .stringValue;
            ...
            while ((measureNode = measureIterator.iterateNext()) !== null) {
                ...
                while ((noteNode = noteIterator.iterateNext()) !== null) {
                    ...
                    while ((tieNode = tieIterator.iterateNext()) !== null) {
                        tmp1 = xmlDoc.evaluate("@type", tieNode, nsResolver, XPathResult.STRING_TYPE, null)
                            .stringValue;
                        //console.log(tmp1);
                        if (tmp1 === "start") {
                            tieStart = true;
                    ...
                    if (rest !== null) {
                        ...
                        voice = xmlDoc.evaluate("voice", noteNode, nsResolver, XPathResult.NUMBER_TYPE, null)
                            .numberValue;
                        octave = xmlDoc.evaluate("pitch/octave", noteNode, nsResolver, XPathResult.NUMBER_TYPE, null).numberValue;
                        noteDuration = xmlDoc.evaluate("duration", noteNode, nsResolver, XPathResult.NUMBER_TYPE, null).numberValue;
                        noteType = xmlDoc.evaluate("type", noteNode, nsResolver, XPathResult.STRING_TYPE, null)
                            .stringValue;
                        noteName = step;
                        if (step !== "") {
                            if (!isNaN(alter)) {
...
function note() {
    ...
    typeString, // → defined in util.js
    //local
    noteNames, getNoteNumber, getNoteName, checkNoteName, getFrequency, createNote, isNoteMode, isBlackKey, pow = Math.pow;
    noteNames = {
    ...
    /*
      ...
      */
    createNote = function () {
        var args = Array.prototype.slice.call(arguments), numArgs = args.length, data, warn = false, error = false, octave, noteName, noteNumber, noteNameMode, frequency, arg0 = args[0], arg1 = args[1], arg2 = args[2];
        // arg: note number
    ...
    getNoteName = function (number, mode) {
        ...
        var octave = Math.floor(number / 12 - 1), noteName = noteNames[mode][number % 12];
    ...
    checkNoteName = function () {
        var args = Array.prototype.slice.call(arguments), numArgs = args.length, arg0 = args[0], arg1 = args[1], length, i, char, name, octave;
...
function parseEvents() {
    "use strict";
    var round = Math.round, precision = Math.pow(10, sequencer.precision), 
    //local
    factor, nominator, denominator, bar, beat, sixteenth, tick, ticksPerBar, ticksPerBeat, ticksPerSixteenth, numSixteenth, millisPerTick, secondsPerTick, millis, bpm;
    // public
    function parse(song, events) {
        var event, numEvents, startEvent = 0, lastEventTick = 0, diffTicks, i;
        numEvents = events.length;
...
function parseTimeEvents() {
    ...
    createMidiEvent, // → defined in midi_event.js
    //local
    ppq, bpm, factor, nominator, denominator, playbackSpeed, bar, beat, sixteenth, tick, ticks, millis, millisPerTick, secondsPerTick, ticksPerBeat, ticksPerBar, ticksPerSixteenth, numSixteenth, timeEvents, numTimeEvents, index;
...
function part() {
    ...
    //private
    getEvent, addEvents, moveEvents, removeEvents, transposeEvents, getEventsAndConfig, reverseByPitch, reverseByTicks, partId = 0, 
    //public/protected
    Part;
    Part = function (name) {
        this.className = "Part";
    ...
    getEventsAndConfig = function (args, part) {
        ...
        var j = 0, i = 0, maxi, e, arg, arg0 = args[0], events = [], config = [];
    ...
    addEvents = function (args, part, relative) {
        ...
        var i, e, newEvents = args.events, ticks = part.ticks, maxi = newEvents.length, track = part.track, eventsById = part.eventsById;
        //console.log(newEvents);
        //for(i = newEvents.length - 1; i >=0; i--){
        for (i = 0; i < maxi; i++) {
            e = newEvents[i];
            if (e.type === sequencer.END_OF_TRACK ||
    ...
    transposeEvents = function (args, part) {
        ...
        var i, e, events = args.events, semi = args.config[0];
    ...
    moveEvents = function (args, part) {
        ...
        var i, e, newTicks, events = args.events, ticks = args.config[0];
    ...
    reverseByPitch = function (part) {
        var notes = part.notes, min = part.lowestNote, max = part.highestNote, on, off, i, note;
    ...
    reverseByTicks = function (part, durationTicks) {
        var notes = part.notes, note, on, off, onTicks, offTicks, i;
    ...
    Part.prototype.copy = function () {
        var part = new Part(copyName(this.name)), partTicks = this.ticks, eventsById = this.eventsById, copies = [], copy, id, event;
    ...
    Part.prototype.reset = function (fromTrack, fromSong) {
        var eventsById = this.eventsById, id, event;
        if (fromSong) {
            this.song = undefined;
        }
        if (fromTrack) {
    ...
    Part.prototype.update = function () {
        ...
        var i, maxi, j, maxj, id, event, noteNumber, note, onEvents, onEvent, firstEvent, lastEvent, stats, noteOnEvents = [], notes = [], numNotes = 0, part = this, partId = this.id, track = this.track, trackId = this.track ? this.track.id : undefined;
        // if(!trackId){
...
function playhead() {
    "use strict";
    var instanceId = 0, range = 10, debug, Playhead, 
    //import
    ...
    Playhead.prototype.calculate = function () {
        var i, event, note, part, position, newParts = [], stillActiveNotes = [], stillActiveEvents = [], collectedParts = [], collectedNotes = [], collectedEvents = [];
        ...
        if (this.type.indexOf("notes") !== -1 || this.type.indexOf("all") !== -1) {
            ...
            for (i = this.activeNotes.length - 1; i >= 0; i--) {
                ...
                //    continue;
                //}
                if (note.noteOn.state.indexOf("removed") === 0 ||
...
function position() {
    ...
    typeString, // → defined in util.js
    supportedTypes = "barsandbeats barsbeats time millis ticks perc percentage", supportedReturnTypes = "barsandbeats barsbeats time millis ticks all", 
    //local
    bpm, nominator, denominator, ticksPerBeat, ticksPerBar, ticksPerSixteenth, millisPerTick, secondsPerTick, numSixteenth, ticks, millis, diffTicks, diffMillis, bar, beat, sixteenth, tick, type, index, returnType = "all", beyondEndOfSong = true, 
    //public (song)
    millisToTicks, ticksToMillis, barsToMillis, barsToTicks, ticksToBars, millisToBars, 
    //private
    checkBarsAndBeats, getDataFromEvent, getPositionData, calculateBarsAndBeats, 
    //protected
    getPosition, checkPosition, fromMillis, fromTicks, fromBars;
    function getTimeEvent(song, unit, target) {
        // finds the time event that comes the closest before the target position
        var timeEvents = song.timeEvents, i, event;
    ...
    fromBars = function (song, targetBar, targetBeat, targetSixteenth, targetTick, event) {
        ...
        var i = 0, diffBars, diffBeats, diffSixteenth, diffTick, lastEvent = song.lastEvent;
        if (beyondEndOfSong === false) {
    ...
    getPositionData = function (song) {
        var timeData, tickAsString, positionData = {};
        switch (returnType) {
            case "millis":
            ...
            case "barsandbeats":
                ...
                tickAsString =
                    tick === 0 ? "000" : tick < 10 ? "00" + tick : tick < 100 ? "0" + tick : tick;
    ...
    checkPosition = function (args) {
        ...
        if (typeString(args) === "array") {
            var numArgs = args.length, position, i, a, positionLength;
            type = args[0];
    ...
    getPosition = function (song, args) {
        ...
        var position = checkPosition(args), millis, tmp, snap;
        if (position === false) {
            console.error("wrong position data");
            return false;
        }
...
function quantizeFixedLength() {
    "use strict";
    var copyObject, // defined in util.js
    floor = Math.floor, round = Math.round, noteFractions = {
        "1": 1 * 4,
        "1.": 1.5 * 4,
        "1..": 1.75 * 4,
        "1...": 1.875 * 4,
    ...
    function quantize(events, value, ppq, history) {
        ...
        var i, event, ticks, quantized, diff, quantizeTicks, quantizeHistory = history || {};
...
function sample() {
    ...
    getEqualPowerCurve, // defined in util.js
    //private
    stopSample, fadeOut, SampleSynth, SampleRelease, SampleSustainRelease, SampleReleasePanning, SampleSustainReleasePanning, Sample = function (config) {
        this.id = getSampleId();
        this.output = context.createGainNode();
    ...
    fadeOut = function (sample) {
        var now = context.currentTime, values, i, maxi;
    ...
    Sample.prototype.unschedule = function (when, cb) {
        var now = context.currentTime, sample = this, fadeOut = when === null ? 100 : when; //milliseconds
    ...
    Sample.prototype.update = function (value) {
        ...
        if (this.startReleasePhase !== undefined &&
...
function samplePack() {
    ...
    context, //defined in open_module.js
    storage, //defined in open_module.js
    parseTime, folder, sampleId, index = 0, SamplePack;
    function parse(samplepack, config) {
        var i, mapping = config.mapping, url, path, name, ext, slash, dot, data, remotePath, sampleData, extension;
        samplepack.samples = [];
    ...
    function store(samplepack) {
        var occupied = findItem(samplepack.localPath, sequencer.storage.samplepacks, true), action = samplepack.action, i, samples, sample;
    ...
    function loadSamples(samples, callback1, callback2) {
        var i = 0, numSamples = samples.length, sample = samples[i];
    ...
    function loadSample(data, callback) {
        var sample, url = data.url, base64 = data.base64;
        sampleId = data.id;
        folder = data.folder;
    ...
    sequencer.addSamplePack = function (config, callback, callbackAfterAllTasksAreDone) {
        var type = typeString(config), samplepack, json, name, folder;
        callbackAfterAllTasksAreDone =
            callbackAfterAllTasksAreDone === undefined ? false : callbackAfterAllTasksAreDone;
        //console.log(config);
        if (type !== "object") {
            if (sequencer.debug >= 2) {
...
function scheduler() {
    "use strict";
    var typeString, // defined in util.js
    objectForEach, // defined in util.js
    context, getTimeDiff, 
    ...
    */
    ...
    Scheduler.prototype.unschedule = function () {
        var args = Array.prototype.slice.call(arguments), events = [], i, e, track, instrument;
        loop(args, 0, args.length, events);
        for (i = events.length - 1; i >= 0; i--) {
            e = events[i];
            track = e.track;
    ...
    Scheduler.prototype.reschedule = function () {
        var i, track, numTracks = this.song.numTracks, tracks = this.song.tracks;
        for (i = 0; i < numTracks; i++) {
            track = tracks[i];
...
function createSequencer() {
    ...
    r = 0, heartbeat, // the heartbeat of the sequencer
    lastTimeStamp, processEventTracks = {}, events, // the events that are currently been processed
    pausedSongs = [], activeSongs = {}, snapshots = {};
    ...
    sequencer.getSnapshot = function (song, id) {
        ...
        id = id || song.id;
        var snapshot = snapshots[id], activeEvents = song.activeEvents, activeNotes = song.activeNotes, activeParts = song.activeParts, nonActiveEvents = [], nonActiveNotes = [], nonActiveParts = [], prevActiveEvents, prevActiveNotes, prevActiveParts, e, n, p, i;
    ...
    sequencer.processEvent = sequencer.processEvents = function () {
        var args = slice.call(arguments), loop, arg, i, maxi, time, contextTime, event, bpm = 60, midiEvent, type, instrument, part, track, secondsPerTick;
        events = [];
    ...
    sequencer.play = function () {
        var args = slice.call(arguments), events = [], parts = [], tracks = [], songs = [], timeEvents = [], i, arg, loop, store = false, song, track, part, bpm, nominator, denominator, instrument;
...
function song() {
    ...
    //private
    _removeTracks, pulse, getArguments, getTrack, addTracks, getPart, getParts, getTimeEvents, setRecordingStatus, _getRecordingPerTrack, songIndex = 0, 
    //protected
    createGrid, 
    //public
    Song;
    Song = function (config) {
        ...
        if (config.timeEvents !== undefined &&
    ...
    addTracks = function (newTracks, song) {
        ...
        var tracksById = song.tracksById, tracksByName = song.tracksByName, addedIds = [], i, part, track;
    ...
    */
    ...
    pulse = function (song) {
        ...
        now = sequencer.getTime() * 1000, diff = now - song.timeStamp, millis = song.millis + diff;
        song.diff = diff;
        //console.log(diff);
    ...
    Song.prototype.startRecording = Song.prototype.record = function (precount) {
        ...
        var userFeedback = false, audioRecording = false, i, track, self = this;
    ...
    Song.prototype.quantize = function () {
        var i, track, arg, type, args = slice.call(arguments), numArgs = args.length, value, historyObject = {};
        //console.log(arguments);
        for (i = 0; i < numArgs; i++) {
            arg = args[i];
            type = typeString(arg);
    ...
    Song.prototype.setRightLocator = function () {
        ...
        var pos = getPosition(this, slice.call(arguments)), previousState = this.illegalLoop;
    ...
    Song.prototype.setPlayhead = function () {
        ...
        var pos = getPosition(this, slice.call(arguments)), millis = pos.millis;
    ...
    Song.prototype.addTrack = function () {
        var args = getArguments(arguments), arg0 = args[0], numArgs = args.length;
    ...
    Song.prototype.getTracks = function () {
        var args = getArguments(arguments), track, i, result = [];
        for (i = args.length - 1; i >= 0; i--) {
    ...
    Song.prototype.setPlaybackSpeed = function (speed) {
        ...
        var ticks = this.ticks, startLoop, endLoop, newPos;
    ...
    Song.prototype.updateTempoEvent = function (event, bpm) {
        ...
        var ticks = this.ticks, percentage = this.percentage;
        event.bpm = bpm;
        this.update(true);
    ...
    Song.prototype.updatePlayheadAndLocators = function (ticks) {
        var newStartPos, newEndPos, startPos = this.loopStartPosition, endPos = this.loopEndPosition, newPos;
        // get the new position of the locators after the update
    ...
    Song.prototype.setTempo = function (bpm, update) {
        var timeEvents = getTimeEvents(sequencer.TEMPO, this), i, event, ticks = this.ticks, percentage = this.percentage, ratio = bpm / timeEvents[0].bpm;
        for (i = timeEvents.length - 1; i >= 0; i--) {
            event = timeEvents[i];
    ...
    Song.prototype.setTimeSignature = function (nominator, denominator, update) {
        var timeEvents = getTimeEvents(sequencer.TIME_SIGNATURE, this), i, event, percentage = this.percentage, ticks = this.ticks;
        for (i = timeEvents.length - 1; i >= 0; i--) {
            event = timeEvents[i];
            event.nominator = nominator;
            event.denominator = denominator;
    ...
    Song.prototype.resetTempo = function (bpm) {
        var firstTempoEvent = getTimeEvents(sequencer.TEMPO, this)[0], timeEvents = this.timeEvents;
    ...
    Song.prototype.resetTimeSignature = function (nominator, denominator) {
        var firstTimeSignatureEvent = getTimeEvents(sequencer.TIME_SIGNATURE, this)[0], timeEvents = this.timeEvents, ticks = this.ticks;
    ...
    Song.prototype.addTimeEvent = Song.prototype.addTimeEvents = function () {
        var events = getArguments(arguments), ticks = this.ticks, i, event, position;
        // console.log(events);
        for (i = events.length - 1; i >= 0; i--) {
            event = events[i];
    ...
    */
    Song.prototype.removeTimeEvent = Song.prototype.removeTimeEvents = function () {
        var tmp = getArguments(arguments), i, maxi = tmp.length, event, ticks = this.ticks, events = [];
        for (i = 0; i < maxi; i++) {
            event = tmp[i];
    ...
    Song.prototype.removeDoubleTimeEvents = function () {
        var events = [], i, event, ticks, type, eventsByTicks = {
            "81": {},
            "88": {},
        };
    ...
    Song.prototype.setDurationInBars = function (bars) {
        var me = this, removedEvents = me.findEvent("bar > " + bars), removedParts = [], changedTracks = [], changedParts = [], dirtyTracks = {}, dirtyParts = {};
        //console.log(removedEvents);
        removedEvents.forEach(function (event) {
            var trackId = event.trackId, partId = event.partId;
    ...
    Song.prototype.setVolume = function () {
        ...
        var args = slice.call(arguments), numArgs = args.length, tracks = [], value, i;
        function loop(data, i, maxi) {
            for (i = 0; i < maxi; i++) {
                var arg = data[i], type = typeString(arg);
                if (type === "array") {
                    loop(arg, 0, arg.length);
                }
                else if (type === "number") {
...
function songEventListener() {
    ...
    // import
    typeString, // defined in util.js
    listenerIndex = 0, addEventListener, removeEventListener, dispatchEvent;
    dispatchEvent = function () {
        var i, tmp, listener, args = slice.call(arguments), numArgs = args.length, song = args[0], type = args[1], params = [];
    ...
    addEventListener = function () {
        var args = slice.call(arguments), listenerId, type = args[0];
        switch (type) {
            case "play":
            case "stop":
            case "pause":
    ...
    removeEventListener = function () {
        var args = slice.call(arguments), tmp, arg0 = args[0], callback = args[1], type, id, filteredListeners = [], i, listener;
        if (arg0.indexOf("_") !== -1) {
            tmp = arg0.split("_");
            type = tmp[0];
            id = arg0;
...
function songFollowEvent() {
    ...
    //private
    getEvents, checkOperatorConflict, 
    ...
    FollowEvent.prototype.updateSong = function () {
        ...
        var i, j, e, eventId, events, tmp, data, listenerIds = [], listeners, listener, listenerId;
    ...
    FollowEvent.prototype.update = function () {
        var i, position = this.song, ticks = position.ticks, events = [], numEvents, event;
    ...
    FollowEvent.prototype.callEventListeners = function (eventId, event) {
        var i, id, tmp, listener, listenerIds = [];
    ...
    FollowEvent.prototype.callListenersPositionRepetitive = function (positionType) {
        var listener, listenerIds = this.allListenersByType.position.repetitive[positionType], me = this;
        if (listenerIds) {
    ...
    FollowEvent.prototype.callListenersPositionConditionalSimple = function (positionType, value) {
        var listener, data, operator, call = false, listenerIds = this.allListenersByType.position.conditional_simple[positionType], me = this;
    ...
    FollowEvent.prototype.callListenersPositionConditionalComplex = function (positionType, value) {
        var listener, value1, value2, operator1, operator2, listenerIds = this.allListenersByType.position.conditional_complex[positionType], me = this;
        if (listenerIds) {
            listenerIds.forEach(function (id) {
                // -> check condition(s)
                listener = me.allListenersById[id];
    ...
    FollowEvent.prototype.callListenersPositionTicks = function (ticks) {
        var tmp = this.allListenersByType.position.ticks, i, maxi = tmp.length, listener;
        for (i = 0; i < maxi; i++) {
    ...
    */
    FollowEvent.prototype.addEventListener = function (type, data, callback) {
        var i, events, event, storeArray, tmp, subtype, listener, listenerId, listenerIds = [], dataType = typeString(data);
    ...
    */
    FollowEvent.prototype.addPositionEventListener = function (data, callback) {
        var tmp, listenerId, listener, millis, searchString = data.split(/[\s+]/g), len = searchString.length, type = searchString[0], operator1 = searchString[1], value1 = searchString[2], operator2 = searchString[3], value2 = searchString[4], value1Type = typeString(value1);
            ...
            case "time":
                ...
                if (!isNaN(value1)) {
                    ...
                    if (value1.length === 1) {
                        ...
                        value1 = getPosition(this.song, ["time", value1[0], value1[1], value1[2], value1[3]])
                            .ticks;
                    }
                    /*
                                    }else if(value1Type === 'array'){
    ...
    FollowEvent.prototype.removeEventListener = function (args) {
        var //args = Array.prototype.slice.call(arguments),
        arg0, numArgs, type, subtype, data, callback, id, ids, tmp, listener, listenerId, event, eventId, eventIds, i, j, k, removeMe, listenerIds, filteredListenerIds = [], removedListenerIds = [], dataType;
        //console.log(args);
    ...
    getEvents = function (type, data) {
        var dataType = typeString(data), events = [], i, e;
        if (dataType !== "array" &&
            data !== undefined &&
...
function songGrid() {
    ...
    positionToGrid, eventToGrid, noteToGrid, gridToSong, // catch all -> may be remove this
    positionToSong;
    ...
    sequencer.positionToSong = sequencer.coordinatesToPosition = sequencer.gridToSong = function () {
        var args = Array.prototype.slice.call(arguments), numArgs = args.length, arg0 = args[0];
    ...
    sequencer.songToGrid = function () {
        var args = Array.prototype.slice.call(arguments), numArgs = args.length, arg0 = args[0], arg1 = args[1];
        switch (numArgs) {
            case 3:
    ...
    eventToGrid = function (event, width, height, song) {
        ...
        var x = (event.millis / song.durationMillis) * width, y, note = sequencer.createNote(event.noteNumber);
...
function songUpdate() {
    ...
    update2 = function (song, updateTimeEvents) {
        ...
        var i, id1, id2, id3, tmp, track, part, event, note, dirtyEvents, dirtyNotes, newEvents = [], changedEvents = [], removedEvents = [], recordedEvents = [], newNotes = [], changedNotes = [], removedNotes = [], newParts = [], changedParts = [], removedParts = [], newTracks = [], changedTracks = [], removedTracks = [], eventsMidiAudioMetronome = [], eventsMidiTime = [], events = [], midiEvents = [], audioEvents = [], notes = [], parts = [], tracks = [], eventsById = {}, notesById = {}, partsById = {};
    ...
    function checkDuration(song, trim) {
        var lastEvent = song.lastEventTmp, position, key;
    ...
    function parseRecordedEvents(song, events) {
        var i, timeData, position, event, time, timestamp = song.recordTimestamp, startMillis = song.recordStartMillis, totalTime = startMillis, maxi = events.length, playhead = song.playheadRecording;
        //if(startMillis < 0){
        ...
        for (i = 0; i < maxi; i++) {
            ...
            event.barsAsString =
                position.bar + ":" + position.beat + ":" + position.sixteenth + ":" + position.tick;
    ...
    function sortEvents(events) {
        var maxi = events.length, i, event, lastTick = -100000, buffer, newOrder = [];
        for (i = 0; i < maxi; i++) {
            event = events[i];
            if (buffer === undefined) {
                buffer = [];
...
function track() {
    "use strict";
    var debug = sequencer.debug, slice = Array.prototype.slice, 
    //import
    createPart, // → defined in part.js
    typeString, // → defined in utils.js
    copyName, // → defined in utils.js
    ...
    function getPartsAndConfig(args, track) {
        args = Array.prototype.slice.call(args);
        var j = 0, i = 0, maxi, part, arg, arg0 = args[0], parts = [], config = [];
    ...
    function getEventsAndConfig(args, track) {
        args = Array.prototype.slice.call(args);
        var j = 0, i = 0, maxi, event, arg, arg0 = args[0], events = [], config = [];
        if (typeString(arg0) === "array") {
            for (i = arg0.length - 1; i >= 0; i--) {
                arg = arg0[i];
    ...
    function addParts(args, track) {
        ...
        var i, j, e, part, eventsById, ticks, move, song = track.song, parts = args.parts, maxi = parts.length, partsById = track.partsById;
        //for(i = parts.length - 1; i >= 0; i--){
        for (i = 0; i < maxi; i++) {
    ...
    function moveParts(args, track) {
        ...
        var newTicks, part, parts = args.parts, ticks = args.config[0], diffTicks = ticks, i;
    ...
    function movePartsTo(args, track) {
        ...
        var i, part, ticks, diffTicks, song = track.song, parts = args.parts, positions = args.config;
        for (i = parts.length - 1; i >= 0; i--) {
            part = parts[i];
    ...
    function removeParts(args, track) {
        ...
        var i, part, removed = [], tobeRemoved = args.parts;
    ...
    function removeEvents(args, track) {
        ...
        var i, part, partId, event, eventsPerPart = {}, removed = [], tobeRemoved = args; //.events;
        for (i = tobeRemoved.length - 1; i >= 0; i--) {
            event = tobeRemoved[i];
    ...
    */
    ...
    Track.prototype.addPartAt = Track.prototype.addPartsAt = function () {
        var args = getPartsAndConfig(arguments, this), config, parts, i, part, ticks;
        if (args === false) {
            return;
        }
        parts = args.parts;
    ...
    */
    ...
    Track.prototype.copyPart = Track.prototype.copyParts = function () {
        var args = getPartsAndConfig(arguments, this), selectedParts, copiedParts = [];
        if (args === false) {
            return;
        }
    ...
    Track.prototype.removePart = function () {
        var args = getPartsAndConfig(arguments, this), removed = removeParts(args, this);
    ...
    Track.prototype.getParts = function () {
        var args = Array.prototype.slice.call(arguments), arg, result = [], loop;
    ...
    Track.prototype.getPartAt = Track.prototype.getPartsAt = function (position) {
        var ticks = getTicksAtPosition(position, this.song), parts = this.parts, selectedParts = [];
        if (ticks === false) {
    ...
    Track.prototype.getPartFromTo = Track.prototype.getPartsFromTo = function (from, to) {
        var parts = this.parts, selectedParts = [], fromTicks = getTicksAtPosition(from, this.song), toTicks = getTicksAtPosition(to, this.song);
        if (fromTicks === false) {
        ...
        parts.forEach(function (part) {
            if ((fromTicks >= part.start.ticks && fromTicks <= part.end.ticks) ||
    ...
    Track.prototype.getPartBetween = Track.prototype.getPartBetween = function (from, to) {
        var parts = this.parts, selectedParts = [], fromTicks = getTicksAtPosition(from, this.song), toTicks = getTicksAtPosition(toTicks, this.song);
    ...
    Track.prototype.copy = function () {
        var track = new Track(copyName(this.name)), part, i, effect, parts = this.parts, copiedParts = [];
        track.song = null;
    ...
    Track.prototype.transposePart = function (part, semi) {
        var stats = part.getStats("noteNumber all"), min = 0, max = 127, semi2;
    ...
    */
    ...
    Track.prototype.getIndex = function () {
        var index = -1, tracks = this.song.tracks, numTracks = tracks.length, track, i;
        if (numTracks > 0) {
            for (i = 0; i < numTracks; i++) {
                track = tracks[i];
                if (track.id === this.id) {
    ...
    */
    ...
    Track.prototype.setMidiInput = function (id, flag) {
        var input, i, midiInputs = this.midiInputs, availableInputs;
    ...
    Track.prototype.setMidiOutput = function (id, flag) {
        ...
        var output = this.song.midiOutputs[id], me = this;
        if (output === undefined) {
            return;
        }
    ...
    */
    ...
    Track.prototype.setAudioRecordingLatency = function (recordId, value, callback) {
        if (this.audio !== undefined) {
            ...
            this.audio.setAudioRecordingLatency(recordId, value, function (recording) {
                ...
                var i, event, sampleId, audioEvents = this.song.audioEvents;
    ...
    Track.prototype.quantize = function () {
        var i, arg, type, args = slice.call(arguments), numArgs = args.length, value, historyObject = {};
        //console.log(arguments);
        for (i = 0; i < numArgs; i++) {
            arg = args[i];
            type = typeString(arg);
    ...
    Track.prototype.undoQuantize = function (history) {
        ...
        var events = history.tracks[this.id].quantizedEvents, numEvents = events.length, i, event;
        for (i = 0; i < numEvents; i++) {
            event = events[i];
...
function transpose() {
    "use strict";
    var // satisfy jslint
    Pitchshift = window.Pitchshift, context, fftFrameSize = 2048, shifter;
    function init() {
    ...
    function transpose(inputBuffer, semitones, cb) {
        ...
        var numChannels = inputBuffer.numberOfChannels, c, input, length, output, outputs = [], shiftValue, i, outputBuffer;
        ...
        outputBuffer = context.createBuffer(numChannels, length, inputBuffer.sampleRate);
        for (c = 0; c < numChannels; c++) {
...
function util() {
    ...
    var // import
    context, // defined in open_module.js
    slice = Array.prototype.slice, mPow = Math.pow, mRound = Math.round, mFloor = Math.floor, mRandom = Math.random, 
    ...
    function getNiceTime(millis) {
        var h, m, s, ms, seconds, timeAsString = "";
    ...
    function copyName(name) {
        var i = name.indexOf("_copy"), copy, numCopies;
        if (i === -1) {
            copy = name + "_copy";
        }
        else {
    ...
    function removeFromArray(tobeRemoved, array) {
        var i, j, maxi, maxj, newArray = [], remove, elementA, elementB;
    ...
    function removeFromArray2(array, callback) {
        var i, maxi = array.length, element, newArray = [];
        for (i = 0; i < maxi; i++) {
            element = array[i];
    ...
    function ajax(config) {
        var request = new XMLHttpRequest(), method = config.method === undefined ? "GET" : config.method, fileSize, promise;
        function executor(resolve, reject) {
    ...
    function ajax2(config) {
        var request = new XMLHttpRequest(), method = config.method === undefined ? "GET" : config.method, fileSize;
    ...
    function findItemsInFolder(path, root, search_subfolders) {
        ...
        var folders = pathToArray(path), numFolders = folders.length, currentFolder, i, folder, searchFolder = folders[numFolders - 1], items = [];
        if (numFolders === 0) {
    ...
    function base64ToBinary(input) {
        var keyStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=", bytes, uarray, buffer, lkey1, lkey2, chr1, chr2, chr3, enc1, enc2, enc3, enc4, i, j = 0;
    ...
    function parseUrl(url) {
        var filePath = "", fileName = url, fileExtension = "", slash, dot, ext;
        url = url.replace(/\/{2,}/g, "/");
        url = url.replace(/^\//, "");
        url = url.replace(/\/$/, "");
        // check if the url has a path and/or an extension
    ...
    function getEqualPowerCurve(numSteps, type, maxValue) {
        var i, value, percent, values = new Float32Array(numSteps);
    ...
    function remap(value, oldMin, oldMax, newMin, newMax) {
        var oldRange = oldMax - oldMin, newRange = newMax - newMin, result;
    ...
    function createSlider(config) {
        var slider = config.slider, message = config.message, label = config.label, sliderWrapper;
        //mouseDownCalls = [],
        //mouseMoveCalls = [],
        //mouseUpCalls = [];
    ...
    function createSlider2(config) {
        var slider = config.slider, message = config.message, label = slider.parentNode.firstChild;
    ...
    function getRandomNotes(config) {
        var i, ticks = 0, events = [], midiEvent, velocity, numNotes, noteNumber, noteLength, minVelocity, maxVelocity, minNoteNumber, maxNoteNumber, ppq;
        //console.log(config);
        config = config || {};
    ...
    function convertPPQ() {
        //oldPPQ, newPPQ, data, ..., ...
        var args = slice.call(arguments), oldPPQ = args[0], newPPQ = args[1], ratio = newPPQ / oldPPQ, i, event;
    ...
    function getWaveformData(buffer, config, callback) {
        var i, maxi, canvas = document.createElement("canvas"), ctx = canvas.getContext("2d"), pcmRight = buffer.getChannelData(0), pcmLeft = buffer.getChannelData(0), numSamples = pcmRight.length, width, // max width of a canvas on chrome/chromium is 32000
        height = config.height || 100, color = config.color || "#71DE71", bgcolor = config.bgcolor || "#000", density, scale = height / 2, sampleStep = config.sampleStep || 50, height, lastWidth, numImages, currentImage, xPos = 0, offset = 0, urls = [], imgElement, imgElements = [];
...
function closeModule(cb) {
    "use strict";
    var ready = false, readyCallbacks = [], context, initMidi, base64ToBinary;
