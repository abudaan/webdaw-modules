...
function audioRecorderWorker() {
    ...
    function createWorker() {
        var data, bufferIndexStart, bufferIndexEnd, planarSamples, interleavedSamples, numFrames, recBuffersLeft, recBuffersRight, sampleRate, numberOfChannels;
        ...
        function getWavFile() {
            ...
            if (numberOfChannels === 1) {
                ...
                interleavedSamples = toInterleavedBuffer(mergeBuffers(recBuffersLeft, numFrames), mergeBuffers(recBuffersRight, numFrames));
            }
        ...
        function toInterleavedBuffer(inputL, inputR) {
            var length = inputL.length + inputR.length, result = new Float32Array(length), index = 0, inputIndex = 0;
            while (index < length) {
        ...
        function toPlanarBuffer(inputL, inputR) {
            var length = inputL.length, result = new Float32Array(length * 2), index = 0, inputIndex = 0;
            while (index < length) {
                result[index++] = inputL[inputIndex++];
            }
            index = 0;
        ...
        function encodeWAV(samples) {
            var bitsPerSample = 16, bytesPerSample = bitsPerSample / 8, buffer = new ArrayBuffer(44 + samples.length * bytesPerSample), view = new DataView(buffer);
            /* RIFF identifier */
        ...
        function getWavFile2() {
            var dataview, i, index = 0, resultLeft, resultRight, mergedBuffersLeft, mergedBuffersRight;
            if (numberOfChannels === 1) {
...
//# sourceMappingURL=audio_recorder_worker.js.map