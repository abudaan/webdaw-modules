  ...
  Instrument.prototype.reschedule = function(song) {
    ...
    for (id in scheduledSamples) {
      if (scheduledSamples.hasOwnProperty(id)) {
        ...
        } else if (note.noteOn.millis >= min && note.noteOff.millis < max && sample.noteName === note.fullName) {
...
function midiStream() {
  ...
  function createStream(buffer) {
    ...
    function readInt32() {
      var result = (buffer[position] << 24) + (buffer[position + 1] << 16) + (buffer[position + 2] << 8) + buffer[position + 3];
      position += 4;
      return result;
    }
